...
def get_local_environment(pdb_file):
    ...
    main_chain_dir = os.path.join("Temp", "temp_" + os.path.basename(pdb_file))
...
if __name__ == '__main__':
    ...
    for i in tqdm(pdb):
        output = os.path.join(output_folder, i.replace("pdb","txt"))
        if os.path.exists(output):
            continue
        else:
            # print(i)
            if dim == 21:
                dim21 = get_local_environment(pdb_file=os.path.join(pdb_path, i))["features"]
                dim21 = dim21.detach().numpy()
                np.savetxt(output,dim21)
            if dim == 100:
                dim100 = get_local_environment(pdb_file=os.path.join(pdb_path, i))["logits"]
